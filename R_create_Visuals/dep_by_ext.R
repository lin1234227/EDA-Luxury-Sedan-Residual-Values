cars <- read.csv("~/Document/data_science/Visual/R_create_Visuals/dataset/dep_by_ext.csv", header=TRUE,  stringsAsFactors=FALSE, row.names = 1)
cars['year'] = 2017-cars['year']
#png(filename="~/Document/data_science/Visual/R_create_Visuals/dataset/tss.png")
cars1<-subset(cars, ext_=='black_')
cars2<-subset(cars, ext_=='white_')
cars3<-subset(cars, ext_=='silver_')
cars4<-subset(cars, ext_=='grey_')
